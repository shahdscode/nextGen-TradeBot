...
"""Base class for Fincast inference. Modified From TimesFm."""
...
class FFmHparams:
  """Hparams used to initialize a FFM model for inference.

  These are the sufficient subset of hparams to configure FFM inference
  ...
  context_len: int = 512 #2048 context window, adjustable for inferencing, 32 - 2048 max.
  horizon_len: int = 128 #fixed for training, adjustable for inferencing, should be same as output_patch_len for training
  input_patch_len: int = 32  #not adjustable, according to pretraining
  output_patch_len: int = 128  #not adjustable, according to pretraining. this is used as a hp when initializing model, for the last ff layer.
  num_layers: int = 50 
  num_heads: int = 16
  model_dims: int = 1280
  per_core_batch_size: int = 32
  backend: Literal["cpu", "gpu", "tpu"] = "gpu"
  quantiles: Sequence[float]  = dataclasses.field(default_factory=create_quantiles)
  use_positional_embedding: bool = False # False, model is trained with false
  ...
  #torch compile model load
  load_from_compile: bool = False
...
class FFmBase:
  """Forecast API for inference. Inspired by google timesfm

  This class is the scaffolding for calling Fincast forecast. To properly use:
    1. Create an instance with the correct hyperparameters of the model.
  ...
  def __init__(self, hparams: FFmHparams,
               checkpoint,
               loading_mode: int = 0) -> None:
    """Initializes the forecast API.
    ...
    self.gating_top_n = hparams.gating_top_n
    self.threshold_train = hparams.threshold_train
    self.threshold_eval = hparams.threshold_eval
    ...
    if self.loading_mode == 0:
      self.load_from_checkpoint_ffm(checkpoint)




  def load_from_checkpoint_ffm(self, checkpoint: str) -> None:
    """Loads a Fincast checkpoint."""
  ...
  def forecast_with_covariates(
    ...
    # Fit models.
    if xreg_mode == "timesfm + xreg":
      # Forecast then fit a model on the residuals.
      ...
      # Fit a model on the targets then forecast on the residuals
      targets = [
          np.array(input_ts)[-train_len:]
