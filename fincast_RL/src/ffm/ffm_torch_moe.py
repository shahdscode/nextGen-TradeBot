...
class FFmTorch(ffm_base.FFmBase):
  ...
  def load_from_checkpoint_ffm(self, checkpoint: str) -> None:
    ...
    def _load_state_dict_safely(
      ...
      """Best-effort torch.load wrapper.

      Notes:
        - `mmap=True` can significantly reduce peak CPU memory usage.
        - Some environments/versions may not support all kwargs, so we retry.
      """
      load_kwargs = {
      ...
      try:
        ...
        # Older torch versions may not accept `weights_only`/`mmap`.
        return torch.load(ckpt_path, map_location=map_location)
      except RuntimeError as e:
        # If mmap-related loading fails, retry without mmap.
        if prefer_mmap and "mmap" in load_kwargs:
          try:
    ...
    if self.backend == "gpu" and not torch.cuda.is_available():
      logging.warning(
          "backend='gpu' was requested, but torch.cuda.is_available() is False. "
          "Falling back to CPU. If you expected GPU, install a CUDA-enabled PyTorch build."
      )

    use_cuda = (self._device.type == "cuda")
    # mmap is only meaningful for CPU storages; CUDA loads should avoid it.
    prefer_mmap = not use_cuda
    map_location = self._device if use_cuda else "cpu"

    # If we're loading tensors directly onto CUDA, move the module first.
    if use_cuda:
    ...
    if self.hparams.load_from_compile: #load from compilled model
      ...
      # Load the state dict
      state_dict = _load_state_dict_safely(
          checkpoint,
          map_location=map_location,
      ...
      loaded_checkpoint = _load_state_dict_safely(
          checkpoint,
          map_location=map_location,
          prefer_mmap=prefer_mmap,
      )
