...
class _NumpyDataset:
  """Minimal stand-in for tf.data.Dataset used by this repo.

  Only implements what's used by the eval pipeline: `.as_numpy_iterator()`.
  """
...
class TimeSeriesdata(object):
  ...
  def tf_dataset(self, mode='train', shift=1):
    ...
    if tf is None:
      logging.warning(
          'TensorFlow failed to import (%s). Falling back to pure NumPy iterator. '
          'Install a supported TensorFlow build to enable tf.data optimizations.',
          getattr(globals().get('_TF_IMPORT_ERROR', None), '__class__', type('E', (), {})).__name__,
      )
      return _NumpyDataset(gen_fn)
