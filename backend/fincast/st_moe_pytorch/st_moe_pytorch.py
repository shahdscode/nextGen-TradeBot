...
import einx  # type: ignore
from einops import rearrange, repeat, reduce, pack, unpack  # type: ignore

try:
    from colt5_attention import topk as maybe_differentiable_topk  # type: ignore
except Exception:  # pragma: no cover
    TopKReturn = namedtuple('TopKReturn', ['values', 'indices', 'coor_descent_values'])

    def maybe_differentiable_topk(x, k, dim=-1, non_differentiable=True, fused=True):  # type: ignore
        values, indices = torch.topk(x, k, dim=dim)
        return TopKReturn(values=values, indices=indices, coor_descent_values=values)
...
class Expert(Module): # custom expert struct, always norm -> forward -> 
    def __init__(self, dim: int):
        super().__init__()

        self.gate_proj = nn.Linear(dim, dim)
        self.down_proj = nn.Linear(dim, dim)
        self.layer_norm = nn.LayerNorm(normalized_shape=dim, eps=1e-6)
...
# class Expert(Module): #custom expert struct, same as TSFM
#     def __init__(
#         self,
#         dim,
#         hidden_mult = 4, #decrease hidden dim to save memory, original 4
#         mult_bias = True,
#         prenorm = False
#     ):
#         super().__init__()
        
#         #add some changes to make the mlp smaller
...
class Experts(Module):
    ...
    def forward(
        ...
        #padding reordering
        # Assume that after rearrangement, x has shape [e, b, n, d]
        # If expert-specific paddings were computed, they should be rearranged similarly.
        if paddings is not None:
            # Rearranging paddings: original expert_paddings has shape [b, e, c]
            # We want to iterate over experts with each expert receiving paddings of shape [b, c]
            expert_paddings = rearrange(paddings, 'b e c -> e b c')
        else:
            expert_paddings = [None] * self.num_experts
        ...
        #old forward without paddings
        # for expert, expert_input in zip(experts, x):
        #     out = expert(expert_input)
        #     outs.append(out)

        #forward process with padding
        for i, (expert, expert_input) in enumerate(zip(self.experts, x)):
            # Use expert-specific padding if available
            pad = expert_paddings[i] if paddings is not None else None
            out = expert(expert_input, paddings=pad)
            outs.append(out)
...
class MoE(Module):
    ...
    def __init__(self,
        ...
        experts = default(experts, lambda: [Expert(dim = dim) for _ in range(num_experts)])
    ...
    def forward(
        ...
        #padding inputs
        if paddings is not None:
            # paddings: [batch, seq_len] and dispatch_tensor: [batch, seq_len, num_experts, capacity]
            expert_paddings = einsum('b n, b n e c -> b e c', paddings, dispatch_tensor)
        else:
            expert_paddings = None


        # feed the expert inputs through the experts.

        expert_outputs = self.experts(expert_inputs, paddings=expert_paddings)
...
class SparseMoEBlock(Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.ff_before = Expert(dim) if add_ff_before else None #no more prenorm
        self.ff_after = Expert(dim) if add_ff_after else None #no prenorm
    ...
    def forward(
        ...
        moe_out, total_aux_loss, balance_loss, router_z_loss = self.moe(self.moe_prenorm(x),
                                                                        paddings=paddings,
                                                                        noise_gates = noise_gates, noise_mult = noise_mult)
